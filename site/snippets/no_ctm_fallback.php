...
<script>if(enhance.ctm()===!1){var lazyloadimgs=document.querySelectorAll("img.lazyload");for(i=0;i<lazyloadimgs.length;i++){var datasrc=lazyloadimgs[i].getAttribute("data-src");lazyloadimgs[i].setAttribute("src",datasrc),lazyloadimgs[i].style.display="block",lazyloadimgs[i].parentNode.style.display="block",lazyloadimgs[i].parentNode.style.paddingBottom="0px"}var lazyloaddivs=document.querySelectorAll("[data-default-bgset]");for(i=0;i<lazyloaddivs.length;i++){var datasrc=lazyloaddivs[i].getAttribute("data-default-bgset");lazyloaddivs[i].style.backgroundImage="url("+datasrc+")"}}</script>
<?php /* ///////////////////// UNMINIFIED! /////////////////////
<script>
	if(enhance.ctm() === false) {
		...
		// see: `responsiveimages.default` width and `thumbs.quality` in config
		var lazyloaddivs = document.querySelectorAll('[data-default-bgset]');
		for(i = 0; i < lazyloaddivs.length; i++) {
			// Set data-bgset as style background
			var datasrc = lazyloaddivs[i].getAttribute('data-default-bgset');
