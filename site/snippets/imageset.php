...
<?php elseif($imageset->outputStyle() === 'bgimage'): /* Background image output mode */ ?>

	<?php if($imageset->count() > 1): /* image set with multiple ratios */ ?>

		<span>A BACKGROUND IMAGE(SET) WITH MULTIPLE RATIOS IS NOT SUPPORTED!</span>

	<?php else: /* image set with single ratio */ ?>

		<span class="bg-image__placeholder<?php if($imageset->option('lazyload')): echo ' lazyload'; endif; ?>" aria-hidden="true"<?php if($imageset->option('placeholder') == 'color'): echo ' style="background-color: ' . $imageset->color() . '"'; endif; ?>></span>

		<?php if($imageset->option('lazyload')): ?>

			<span class="bg-image__image lazyload" <?= $imageset->srcAttributes(['lazyload' => false]) ?> data-sizes="auto"></span>

			<?php if($imageset->option('noscript')): ?>
				<noscript>
					<span class="bg-image__image" style="background-image:url(<?php echo $imageset->src(); ?>);"></span>
				</noscript>
			<?php endif; ?>
